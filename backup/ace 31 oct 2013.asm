...
L0028:  INCUB	R3						// increase high byte
...
//		LD		H,A						// place back in H.
		STR		R3,0x3C18				// set system variable RAMTOP.
...
L004B:  BL		L0A24					// routine CLS.
		...
		STR		R0,(0x2700)				// make location after screen zero.
...
L005F:
		...
		INCLB	R3						// increment low byte of address
...
// a carriage return to input buffer i.e. lower screen memory.
		...
		STR		R3,(0x3C22)				// set ENDBUF - end of logical line
		STR		R3,(0x3C20)				// set the CURSOR
		...
		STR		R3,(0x3C1E)				// set INSCRN to this position.
		MOV		PC, LR					//Return
...
L0198:  EX		R0,R6						// preserve the output character.
		...
		LDR		R3,($3C22)				// fetch ENDBUF end of logical line
...
L01A6:  LDR		R2,(0x3C24)				// fetch start of buffer from L_HALF
		LDR		R3,=0xDBA0				// $0000 - $2460
		ADD		R3,R2					//
		BCC		L01E4					// forward to exit if buffer full.


		LDR		R3,(0x3C1C)				// fetch position SCRPOS for upper screen
...
L0211:  LD		HL,($3C20)				// fetch CURSOR position
		...
		RET
*/
L0211:  LDR		R3,($3C20)				// fetch CURSOR position
		INC		R3						// and increment it.

		LDR		R2,($3C22)				// fetch ENDBUF - end of current line.
		AND		R0,R0,R0				// prepare to subtract.
		SBC		R0,R3,R2				// test
		MOVEQ	PC,LR						// return if zero - CURSOR is at ENDBUF

		ADD		R3,R2					// else reform the pointers.
		STR		R3,($3C20)				// update CURSOR
		LDR		R0,[R3]					// fetch character at new position.
		DEC		R3						// decrement
		STR		R0,(HL)					// and insert where cursor was.
		MOV		PC,LR						// ret.
...
L0225:  LD		HL,($3C20)				// fetch CURSOR
		...
		LD		($3C20),HL				// update CURSOR
*/
L0225:  LDR		R3,($3C20)				// fetch CURSOR
...
//		LD		L,E						//
		...
		STR		R2,($3C20)				// else update CURSOR
 //		LD		A,B						// check for count of characters
 //		OR		a,a,C					// being zero
		CMP		r1,#0
...
L0254:  LDR		R3,($3C1E)				// fetch INSCRN start of current line.
		LDR		R2,($3C24)				// fetch L_HALF start of buffer.
		...
		LDR		R3,($3C1E)				// fetch INSCRN leftmost location of
										// current line.
...
L0269:  ADD		R3,R3,R2					// subtract 32
		...
		STR		R3,($3C1E)				// update INSCRN

		BL		L02F4					// find endbuf

		STR		R3,($3C20)				// set CURSOR
...
// -> from interrupt
L0282:  LDR		R3,($3C20)				// CURSOR

		LDR		R0,($3C28)				// STATIN
		RRA								// ignore bit 0
		STR		R3,$97					// pixel cursor.
		RRA								// test bit 1 - CAPS
...
*/
// -> called by retype
L02EA:  LDR		R3,($3C24)				// fetch L_HALF
...
L0302:  LD		HL,($3C22)				// system variable ENDBUF
		...
		RET								// return with zero flag set if cursor
										// at EOL.
*/
L0302:  LDR		R3,(0x3C22)				// system variable ENDBUF
		LDR		R2,(0x3C20)				// system variable CURSOR
		AND		R0,R0,R0				// prepare to subtract.
...
L0310:  BL		L0336					// routine KEY_SCAN
		...
		LDRB	R1,R0					// save key in B

		LDRH	R3,(0x3C26)				// load L with KEYCOD - last key pressed
										// load H with KEYCNT - debounce counter
...
L0325:  DECUB	R3						// decrement the counter.
		...
		LDRUB	R3,0x04					// else set counter to four.

L0331:  LDR		R0,L					// pick up previous key.

L0332:  STR		R3,($3C26)				//  update KEYCOD/KEYCNT
...
*/
		LDRLB	R3,R0					// save key range value in L
		LDR		R0,E					// fetch the original port reading.
		ORR		R0,R0,#0x03				// cancel the two shift bits.

		LDRLB	R2,0xFF					// set a flag to detect multiple keys.
...
L0359:  SUB		R0,R0,#0x08						// subtract 8 from key value
		...
		LDRLB		R2,R0					// transfer key value to E.
		BNE		L036B					// forward to abort if more than one key
										// is pressed in the row.

L0362:  DECLB	R3						// decrement the key value for next row.
...
L036B:  LDRLB	R2,0xFF					// signal invalid key.
...
*/
L03F5:  LDRUB	R1,R0					// save the character in the B register.

		LDR		R3,($3C29)				// fetch possible vector from EXWRCH
										// (normally 0)
		CMP		R3,#0x0					// test for
...
L041C:  STR		R3,($3C1C)				// update SCRPOS
...
// now calculate the number of characters to scroll in the upper display.
		...
		LDR		R3,(0x3C24)				// fetch L_HALF the start of input buffer
...
L042F:  AND		R0,R0,R0				// prepare for true subtraction.
		...
		LDRUB	R1,0x20					// blank a line of 32 characters

L043D:  DEC		R3						// decrement screen address.
		STRLB	0x20,[R3]				// insert a space character
		DJNZ	L043D					// and loop for all 32 characters
...
L0462:  LDR		R2,(0x3C37)				// system variable STKBOT.
		RST		0x10						// push word DE
...
L0536:  DEFW	L0538					// headerless 'code field'
.align
L0538:  LDR		R0,($3C3E)				// fetch system variable FLAGS
...
L054A:  LDR		R0,0x0D					// prepare a carriage return.
		RST		0x08						// and PRINT also.
.align
L054D:  B		(R5)					// to 'next'.
...
L057A:  BL		L02EA					// routine sets logical line.
		...
		STRLB	0xBF,[R3]				// the inverse [?] character
...
L05AD:  RST		0x18						// pop word DE
		...
		LDRUB	R1,0xFD					// the count is 253.

L05B3: 	STRLB	0x20,[R3]				// insert a space in pad.
		DEC		R3						// decrement the address.
...
L05DF:  LDRLB	R2,0x20					// set a space as the skip character.

// =>called with E holding delimiter.
//
L05E1:  LDR		R3,(0x3C24)				// fetch L_HALF - start of screen buffer.
		STR		R3,(0x3C1E)				// make INSCRN start of logical line the
										// same.
...
L0600:  PUSH	{R2}					// save delimiter
		...
		LDR		R2,($3C24)				// else set DE from L_HALF
		BL		L07FA					// routine SPACE_FILL (DE-HL)
		STR		R3,($3C24)				// set L_HALF to next line
...
L062F:  LDR		R0,=0x0D					// prepare a newline
		...
		LDRLB	R1,0x00					// set a flag for 'do all names'.
...
L067F:  DEC		R3						// point to high byte of 'link field'
		...
		LDRUB	R3,R0					// 'name length field' to HL pointer.
...
L0742:  XOR		A
		...
		RET
*/
L0740:  LDRUB	R1,0x06					// six nibbles
...
*/
.align
L084E:  LDR		R3,($3C3B)				// fetch SPARE - start of Spare Memory.
		...
		STR		R3,($3C3B)				// update the system variable SPARE to
										// a location two bytes less than it was.
		MOV		PC,LR					// return.
// -----------------------------------------
...
L098F:  LDR		R3,=0x27FF				// end of pad
		STR		R3,(0x3C1A)			// update system variable HLD
		B		(R5)					// to 'next'.
.LTORG
// -------------
// THE '#>' WORD
...
L0A24:  LDR		R2,=0x26FF				// point destination to end of video
										// memory.
		LDR		R3,(0x3C24)				// set HL to first byte of input buffer
										// from system variable L_HALF.
										// (at initialization $26E0).
...
L0A5F:  LDR		R3,(0x3C1A)				// HLD
		DECLB	R3
		BEQ		L0A69					// forward when full

		STR		R3,($3C1A)				// update HLD
...
*/
L0AB1:  LDR		R3,(0x3C3B)				// set pointer from system variable SPARE
...
L0ABE:  LDRLB	R2,0x00				// signal not scientific notation.
...
L0ACA:  STRLB	0x41,[R3]				// make Data Stack exponent +1
		INC		R0						// restore true exponent byte
		LDRLB	R2,R0					// transfer to E.
...
// exponent is negative so decimal point comes first. e.g. .001
		...
		LDRUB	R1,R0						// result of subtraction to B.
		INCUB	R1						// B is now one less than count of
										// leading zeros.
...
L0B05:  SUB		R0,#0x41				// convert to signed 8-bit.
		LDRLB	R3,R0						// low order byte to L.
		SBC		R0,R0						// $FF negative or $00 positive
		LDRUB	R3,R0						// set the high order byte.
...
L0B1B:  RST		0x18						// pop word DE
		...
		STR		R3,(0x3C1C)				// update system variable SCRPOS
...
L0B28:  ADD		R0,#0x20
		...
		LDRLB	R2,R0
		ADD		R3,R2
		LDR		R2,(0x3C24)				// fetch start of lower half from L_HALF
		SBC		R3,R2
		ADD		R3,R2
		RET		C
...
L0BDD:  BL	L0336						// routine KEY-SCAN
		...
		LDRLB	R2,R0					// transfer the key code to E.
		LDRUB	R1,0x00					// make high order byte zero.
...
L0BED:  BL		L084E					// stk_to_bc
		LDRUB	R2,00					// make high order byte zero.
...
L0C21:  LDR		R0,0x00					// make accumulator zero.
		LDRUB	R2,R0					// set D to zero
		RLA								// pick up carry (1/0)
		LDRLB	R2,R0					// set DE to one or zero
		RST		0x10						// push word DE
...
L0F1D:  INC		R2						// increase source
		...
		STR		R3,($3C39)				// store this location in SPARE
...
L0F36:  LDR		R3,($3C37)				// fetch STKBOT
		LDR		R2,($3C39)				// fetch SPARE
...
//		LD		L,A						//
		MOV		R3,#0
		STR		R3,($3C39)				// update system variable SPARE to zero
...
// now increase the two data stack pointers.
		...
		LDR		R3,(0x3C37)				// fetch value of STKBOT
		ADD		R3,R1					// add required room.
		STR		R3,(0x3C37)				// update STKBOT.

		LDR		R3,(0x3C3B)				// fetch value of SPARE
		PUSH	{R3}					// take a copy of 'old' value
		ADD		R3,R1					// add required room.
		STR		R3,(0x3C3B)				// update SPARE.
...
*/
.align
L1013:  LDRUB	R1,0x01					// counter - one word to push
...
L104D:  POP		{R3}						// retrieve the 'Next Word' pointer.
		...
		LDRUB	R2,0x00				// set high order byte to zero.

		LDRUB	R1,0x01				// set counter to 1.
...
*/
L1066:  LDRUB	R1,0x02					// set counter to two
...
*/
L1108:  BIT		6,(IX+0x3E)				// test FLAGS - compiler mode ?
...
L11AD:  LD		HL,($3C33)				// CONTEXT
		...
		JP		(IY)					// to 'next'.
*/
L11AD:  LDR		R3,(0x3C33)				// CONTEXT
		STR		R3,(0x3C31)				// CURRENT
		B		(R5)					// to 'next'.
// ---
/*
...
L13FF:  BL		L0F2E					// blank stack
		...
		INC		R3
		STR		R3,(0x2705)				// store in pad

		PUSH	{R3}						// (*)

		BL		L15C0					// get 'name field' address

		STR		R3,(0x270D)				// name field addr
		STR		R1,(0x2707)				// parameter field addr
		STR		R2,(0x270B)				// length field value
...
L1568:  LD		HL,($2701)				// first bytes of pad.
		...
		RET								// return
*/
L1568:  LDR		R3,(0x2701)				// first bytes of pad.
		AND		R0,R0,R0				//
...
L15A2:  DEC		R2
		...
		LDRUB	R3,0x00					// make high byte zero
...
L15FB:  POP		{R3}					// drop return address - points to byte
		...
		LDRLB	R3,R0					// now R3 holds the read word
		...
		LDRUB	R2,0x00					// a 1 byte relative jump.
...
L163A:  LD		HL,($3C31)				// CURRENT
		...
		DEFB	$FF						// No error
*/
L163A:  LDR		R3,(0x3C31)				// CURRENT
		LDR		R2,(0x3C33)				// CONTEXT
		AND		R0,R0,R0
		SBC		R3,R2
...
*/
.align
L1708:  LDR		R0,(0x3C14)				// LISTWS2
		STR		R0,(0x3C15)				// LISTWS3
...
L17D4:  LD		(IX+$15),$FF			// LISTWS3
		...
		RET								// return.
*/
L17C1:  LDR		R0,(0x3C15)				// LISTWS3
		AND		R0,R0,R0
		RET		M

		PUSH	{R1}					// preserve R1
		LDRUB	R1,R0					// transfer count to B

		LDR		R0,0x0D					// carriage return.
		RST		0x08						// print_ch

		INCUB	R0						// test indentation.
		DECUB	R1						//
		BEQ		L17D4					//
.align
L17CF:  LDR		R0,0x20					// a space character
		RST		0x08						// print_ch
...
*/
.align
L1A51:  LDR		R0,(0x2302)				// length of word in pad
		AND		R0,R0,R0
...
//
		...
		LDR		R3,(0x3C3B)				// fetch end of data stack+1 from SPARE.
...
// update system variable SPARE - this could be deferred.
		...
		STR		R3,(0x3C3B)				// update system variable SPARE
...
L1B13:  STRB	R0,(0x3C02)				// FP_WS_02				see L1C2F
		...
		STR		R1,(0x3C00)				// store the exponents at start of FP_WS
...
L1B22:  LDR		R0,0x09
		...
		LDRUB	R1,R0					// set shift counter to nine. i.e clear.

L1B28:  LDRLB	R1,0x04					// four bytes
		INC		R3
		INC		R3
...
L1B43:  PUSH	{R1}					// preserve the two
		...
		LDRUB	R1,0x04					// set byte counter to four.
		AND		R0,R0,R0				// clear carry.
...
L1B53:  LDRLB	R1,0x01					// signal the operation is addition.
...
L1B55:  PUSH	{R3}					// preserve the
		...
		LDRUB	R1,R0					// transfer R.H. nibble to B

		LSCLB	R1
		EOR		R0,R0,SCRATCH1			// A now has L.H. nibble.
		LDRLB	R1,R0					// place in C.
/*
...
// imagine we started with ninety-nine so C = 1001 0000  and B = 0000 1001
		...
		LDRLB	R1,R0					//	binary multiplier in C

// note that for simple addition C is unchanged and still contains 1.

		LDRUB	R1,0x04					// four bytes to consider
...
// else perform R3=R3+R2 in BCD.
		...
		DAA								// DAA.
		LDRLB	R3,R0					// result in L and carry.

		LSCUB	R3
		LDR		R0,SCRATCH1				// fetch high byte possibly 1 from carry
		LSCUB	R2
		ADC		R0,SCRATCH1				// add in any carry from above (D=0)
		DAA								// comes into play with multiplication.
		LDRUB	R3,R0					// result to H.

L1B83:  INCLB	R1						// test the counter for zero.
...
L1BB3:  BL	L1AF4						// PREP_FP
		...
		LSCLB	R1
		LDR		R0,SCRATCH1				// take exponent of second number (f2).
		LSCUB	R1
...
L1BC1:  LDRUB	R1,R0					// put positive subtraction result in B.
...
L1BC9:  LDRUB	R1,0x02					// two floating point numbers to consider

		LDRLB	R1,(IX+0x02)			// FP_WS_02
...
*/
		...
		LDRUB	R1,(R4+0x02)				// place result sign in  FP_WS_02
...
L1C0C:  LDR		R0,[R2]					// load the 4th byte to accumulator.
		...
		LDRUB	R1,R0					// nibble count to B.

		LDRB	SCRATCH1,(R4+0x00)
		ADD		R0,R0,SCRATCH1			// add count to FP_WS_00 the result
		STR		R0,(0x3C00)				// exponent and place back in FP_WS_00.
...
*/
L1C21:  LDR		R0,(0x3C00)				// fetch result exponent from FP_WS_00
		...
		LDRUB	R1,R0					// save unsigned exponent in B.
/*
...
L1C4D:  BL	L1AF4						// routine PREP_FP prepares the two
		...
		LDRUB	R1,0x03					// count three bytes - six nibbles.
/*
...
L1C5D:  LDRLB	R1,[R3]					// fetch BCD pair to C
		...
		LDR		R1,(0x3C00)				// fetch raw exponents from FP_WS_00/01
		LSCUB	R1
		LDR		R0,SCRATCH1				// add the exponents
		LSCLB	R1
		ADD		R0,SCRATCH1				// together.

		SUB		R0,R0,#0x42				// adjust for sign

		STR		R0,(0x3C00)				// put the result back in FP_WS_00.
...
L1D18:  LDRUB	R2,R0					// exponent byte to D.
...
*/
L1D24:  LDR		R3,(0x3C3B)				// fetch value from SPARE.
...
//		LDR		C,L						//
		...
		LDRLB	R1,R0						// leftmost nibble from mantissa to C.
		LDRUB	R1,0x00					// prepare to add just the nibble.
...
L1D5B:  RST		0x18					// pop word off stack to R2
		...
		LSCUB	R1
		LDRUB	R2,SCRATCH1
		LDRLB	R2,SCRATCH1				// initialize R2 to zero.
...
L1D62:  ADD		R3,R3,R3				// double
		...
		LDR		SCRATCH1,=0x46
		LDRUB	R2,SCRATCH1					// exponent byte   +6

		LSCUB	R1
		LDRLB	R2,SCRATCH1				// low byte
